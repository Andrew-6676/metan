	...
	<div class="rep_wrapper">
		...
		<table>
			...
			foreach ($data['rec_doc_data'] as $row) {
				...
				$tr .= '</td>';
				$tr .= '<td>';
				$tr .= number_format($row->quantity, '2', '.', ' ');
				$tr .= '</td>';
				$tr .= '<td>';
				$tr .= number_format($row->cost, '2', '.', ' ');
				$tr .= '</td>';
				$tr .= '<td>';
				$tr .= number_format($row->markup, '2', '.', ' ');
				$tr .= '</td>';
				$tr .= '<td>';
				$tr .= number_format($row->cost * (1 + $row->markup/100), '2', '.', ' ');
				$tr .= '</td>';
				$tr .= '<td>';
				$tr .= number_format($row->quantity * $row->cost * (1 + $row->markup / 100), '2', '.', ' ');
				$tr .= '</td>';
				$tr .= '<td>';
				$tr .= $row->vat; //number_format($row->vat, '2', '.', ' ');
				$tr .= '</td>';
				$tr .= '<td>';
//				$tr .= number_format($row->vat / 100 * $row->quantity * $row->cost * (1 + $row->markup / 100), '2', '.', ' ');
				$tr .= number_format($row->vat / 100 * $row->quantity * $row->price, '2', '.', ' ');
				$tr .= '</td>';
				$tr .= '<td>';
				$tr .= number_format($row->price, '2', '.', ' ');
				$tr .= '</td>';
				$tr .= '<td>';
			...
			$tr .= number_format($data['rec_doc']->sum_with_markup, '2', '.', ' ');
			$tr .= '</td>';
			$tr .= '<td class="no_border">';

			$tr .= '</td>';
			$tr .= '<td class="no_border r">';
			$tr .=      number_format($data['rec_doc']->sum_vat2, '2', '.', ' ');
			$tr .= '</td>';
			$tr .= '<td class="no_border">';
			$tr .= '</td>';
			$tr .= '<td class="no_border r">';
			$tr .=      number_format($data['rec_doc']->sum_price, '2', '.', ' ');
			$tr .= '</td>';
			$tr .= '<td class="no_border">';
			$tr .= '</td>';
			$tr .= '</tr>';
